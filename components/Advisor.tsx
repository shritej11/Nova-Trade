  ...
  if (!hasActivity) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center max-w-md mx-auto">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Your AI Tutor is Ready</h2>
  ...
  return (
    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 pb-10">
      ...
      <div className="lg:col-span-3 flex justify-between items-center mb-2">
        <div>
           <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Personalized Investment Insights</h2>
           <p className="text-slate-500 dark:text-slate-400 text-sm">Deep learning analysis of your portfolio and trading behavior.</p>
        </div>
      ...
      {report && (
        <>
          ...
          <Card className="lg:col-span-1 bg-gradient-to-b from-white to-slate-50 dark:from-slate-800 dark:to-slate-900">
             <div className="flex items-center gap-2 mb-6">
               <ChartBarIcon className="h-5 w-5 text-accent" />
               <h3 className="font-bold text-slate-900 dark:text-white">Portfolio Health</h3>
             </div>
             ...
             <div className="space-y-6">
                <div>
                   <div className="flex justify-between text-sm mb-2">
                      ...
                      <span className="text-slate-900 dark:text-white font-bold">{report.riskProfile}</span>
                   </div>
                ...
                <div>
                   <div className="flex justify-between text-sm mb-2">
                      <span className="text-slate-500 dark:text-slate-400">Diversification</span>
                      <span className="text-slate-900 dark:text-white font-bold">{report.diversificationScore >= 70 ? 'Excellent' : report.diversificationScore >= 40 ? 'Moderate' : 'Poor'}</span>
                   </div>
                   <div className="h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                      <div 
          ...
          <Card className="lg:col-span-2">
            <div className="flex items-center gap-2 mb-6">
               ...
               <h3 className="font-bold text-slate-900 dark:text-white">Smart Suggestions</h3>
            </div>
               ...
               {report.portfolioInsights.map((insight, idx) => (
                  <div key={idx} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800">
                     <div className="flex items-start gap-3">
                        {insight.type === 'warning' && <ExclamationTriangleIcon className="h-5 w-5 text-red-500 dark:text-red-400 flex-shrink-0" />}
                        {insight.type === 'suggestion' && <ArrowPathIcon className="h-5 w-5 text-blue-500 dark:text-blue-400 flex-shrink-0" />}
                        {insight.type === 'good' && <CheckCircleIcon className="h-5 w-5 text-emerald-500 dark:text-emerald-400 flex-shrink-0" />}
                        <div>
                           <h4 className="text-slate-900 dark:text-white font-bold text-sm mb-1">{insight.title}</h4>
                           <p className="text-slate-600 dark:text-slate-400 text-xs leading-relaxed">{insight.description}</p>
                        </div>
                     </div>
                  </div>
               ))}
          ...
          <Card className="lg:col-span-3 bg-accent/10 border-accent/20">
             <div className="flex items-start gap-4">
                ...
                <div>
                   <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">Today's Learning Path: {report.learningPath.topic}</h3>
                   <p className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed">{report.learningPath.content}</p>
                </div>
             </div>
          </Card>
          ...
          <div className="lg:col-span-3">
             ...
             <div className="space-y-4">
                {user.tradeHistory.slice(-5).reverse().map((trade) => {
                   ...
                   return (
                      <Card key={trade.id} className="group hover:border-slate-300 dark:hover:border-slate-600 transition-colors">
                         <div className="flex flex-col md:flex-row md:items-start gap-6">
                            <div className="min-w-[150px]">
                               ...
                               <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">{new Date(trade.timestamp).toLocaleString()}</div>
                               <div className="text-sm text-slate-900 dark:text-white font-mono">
                                  {trade.quantity} @ ₹{trade.price.toFixed(2)}
                               </div>
                               {trade.profitLoss !== undefined && (
                                  <div className={`text-xs font-bold mt-1 ${trade.profitLoss >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                                     {trade.profitLoss >= 0 ? '+' : ''}₹{trade.profitLoss.toFixed(2)}
                                  </div>
                               )}
                            </div>
                            
                            <div className="flex-1 border-l border-slate-200 dark:border-slate-800 pl-0 md:pl-6 pt-4 md:pt-0">
                               {review ? (
                                  <>
                                     ...
                                     <p className="text-slate-500 dark:text-slate-400 text-xs bg-slate-100 dark:bg-slate-900/50 p-2 rounded italic">
                                        <span className="text-yellow-600 dark:text-yellow-500 not-italic font-bold">Tip: </span>
